...
import CreatorDeletePopper from "../../../Popups/delete-creator.js";
import CreatorEditPopper from "../../../Popups/edit-creator.js";
...
const AdminCreators = () => {
  ...
  const fetchAcceptedCreators = useCallback(async () => {
    //saves the function so there is no need for compiler to create new instance of it every time it needs it
    try {
      const accepted_creators_response = await axios.get(
        "http://localhost:8100/api/users/getCreators/data"
      ); //axios returns a response object with a data property, we then use .data to get it
      // console.log(
      //   "This is the accepted creators response: ",
      //   accepted_creators_response
      // ); //for checking
              ...
              accepted_creators ? (
                accepted_creators.map((creator) => (
                  ...
                  >
                    ...
                    <StyledTableCell align="right">
                      {creator.dob.split("T")[0]}
                    </StyledTableCell>
                    <StyledTableCell align="right">
                      <CreatorEditPopper
                        creator={creator}
                        cb={fetchAcceptedCreators}
                      />
                    </StyledTableCell>
                    <StyledTableCell align="right">
                      <CreatorDeletePopper
