...
const RegisterRequestDeletePopper = ({row, cb}) => {
  ...
  const handleRegisterRequestDelete = async (id) => {
    try {
      await axios.delete(`http://localhost:8100/api/users/${id}`);
    } catch (error) {
      console.log("error deleting register_request: ", error);
    } finally {
      cb();
    }
  };
          ...
          <Fade {...TransitionProps}>
            <Box sx={{ border: 1, p: 1, bgcolor: "#333333" }}>
              ...
              <form className="button-container-main container d-flex">
                ...
                <button className="btn btn-warning" type="reset" onClick = {() => handleRegisterRequestDelete(row.id)}>
                  <b>Yes</b>
                </button>
              </form>
            </Box>
...
export default RegisterRequestDeletePopper;
